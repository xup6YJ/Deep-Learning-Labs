...
class DQN:
    ...
    def load(self, model_path, checkpoint=False):
        ...
        print('Successfully load model from %s' % model_path)
...
def train(args, agent, writer):
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if total_steps and total_steps % args.eval_freq == 0:
                """You can write another evaluate function, or just call the test function."""
                
                avg_test_reward = test(args, agent, writer)
                ...
                agent.save(args.model + "dqn_" + formatted_time + "_[episode:" + str(episode) + "]_[total_steps:" + str(total_steps) + "]_[" + str(avg_test_reward) + "].pt")
...
def main():
    ...
    parser.add_argument('--eps_decay', default=2000000, type=float)
    ...
    parser.add_argument('-tmp', '--test_model_path', default='ckpt/dqn_20230822_180702_[episode:53181]_[total_steps:7600000]_[394.8].pt')
    ...
    if args.test_only:
        ...
        highest_avg_award = 400
        test_run = 1
        highest_score_run = 1

        while True:
            print("Testing run " + str(test_run))

            avg_reward = test(args, agent, writer)

            if avg_reward >= highest_avg_award:
                print("Highest average reward: " + str(avg_reward))
                break
