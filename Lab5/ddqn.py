...
class DQN:
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            a_next = torch.argmax(self._behavior_net(next_state), 1).view(-1, 1)
            q_next = self._target_net(next_state).gather(1, a_next)
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                writer.add_scalar('Test/Episode Reward', total_reward, n_episode+1)
                print('Episode {}: {}'.format(n_episode+1, total_reward))
                break
...
def main():
    ...
    parser.add_argument('-m', '--model', default='double-dqn.pth')
    parser.add_argument('--logdir', default='log/double-dqn')
    # train
